//! Definitions to read and write consensus signing keys.
//!
//! Ported from `tempo-commonware-node-config`.
...
impl SigningKey {
    /// Unwraps into the inner `PrivateKey`.
    pub fn into_inner(self) -> PrivateKey {
        self.inner
    }

    /// Reads a hex-encoded ed25519 private key from a file.
    ...
    /// Parses a hex-encoded ed25519 private key.
    ...
    /// Returns the public key derived from this signing key.
...
/// Error type for [`SigningKey`] operations.
...
/// A BLS12-381 threshold signing share.
...
impl SigningShare {
    /// Unwraps into the inner `Share`.
    pub fn into_inner(self) -> Share {
        self.inner
    }

    /// Reads a hex-encoded BLS12-381 share from a file.
    ...
    /// Parses a hex-encoded BLS12-381 share.
    ...
    /// Writes the signing share to a file.
...
/// Error type for [`SigningShare`] operations.
#[derive(Debug, thiserror::Error)]
#[error(transparent)]
