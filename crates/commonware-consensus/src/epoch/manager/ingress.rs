//! Epoch manager ingress (mailbox and message types).
//!
//! Ported from Tempo with the same interface so that the DKG manager
//! can instruct the epoch manager to enter/exit epochs.

use commonware_consensus::{marshal::Update, types::Epoch, Reporter};
...
impl Message {
    fn in_current_span(activity: impl Into<Content>) -> Self {
        Self { cause: Span::current(), content: activity.into() }
    }
}
...
impl Reporter for Mailbox {
    ...
    async fn report(&mut self, activity: Self::Activity) {
        if self.inner.unbounded_send(Message::in_current_span(activity)).is_err() {
            error!(
